...
import { Column, Flex, Icon, Input, Option, Text } from ".";
import styles from "./styles/Kbar.module.scss";
...
export const KbarTrigger: React.FC<KbarTriggerProps> = ({ onClick, children, ...rest }) => {
  return (
    <Flex onClick={onClick} data-id="KbarTrigger" {...rest}>
      {children}
    </Flex>
  );
};
  ...
  return (
    ...
    >
      <Column
        ...
        overflow="hidden"
        shadow="l"
        className={` ${isClosing ? styles.closing : ""}`}
