...
const StylePanel = forwardRef<HTMLDivElement, StylePanelProps>(({ ...rest }, ref) => {
  const [selectedShape, setSelectedShape] = useState(styleConfig.border);
  const [brandColor, setBrandColor] = useState(styleConfig.brand);
  const [accentColor, setAccentColor] = useState(styleConfig.accent);
  const [neutralColor, setNeutralColor] = useState(styleConfig.neutral);
  const [theme, setTheme] = useState(styleConfig.theme);
  const [solid, setSolid] = useState(styleConfig.solid);
  const [solidStyle, setSolidStyle] = useState(styleConfig.solidStyle);
  const [transition, setTransition] = useState(styleConfig.transition);
  const [scaling, setScaling] = useState(styleConfig.scaling);
  const [surface, setSurface] = useState(styleConfig.surface);
